...
const { prepareBranches, importBranches } = require('../src/import/branchCandidates');

function inputFile() {
  const index = process.argv.indexOf('--file');
  if (index < 0 || !process.argv[index + 1]) throw new Error('用法：node scripts/import-branches.js --file branches.json');
  return path.resolve(process.argv[index + 1]);
}

async function main() {
  const file = inputFile();
  const records = prepareBranches(JSON.parse(await fs.readFile(file, 'utf8')));
  ...
  try {
    const result = await withTransaction(async client => {
      const stats = await importBranches(client, records);
      await client.query(
    ...
    console.log(`商家候选导入完成：${result.total} 条，新增 ${result.inserted}，更新 ${result.updated}`);
  } catch (error) {
